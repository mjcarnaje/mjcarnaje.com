...
$`\frac{dS}{dt}`$ = (1-p)αN - $`\frac{β\sqrt{S}I}{N}`$ - μ$`\sqrt{S}`$,

$`\frac{dI}{dt}`$ = $`\frac{β\sqrt{S}I}{N}`$ - (γ+μ)I

$`\frac{dS}{dt}`$ = pαN + γI - μR

### Where

- **$`\frac{dS}{dt}`$** - Susceptible over time

- **$`\frac{dI}{dt}`$**- Infected person over time

- **$`\frac{dS}{dt}`$**- Recovered over time
